...
def cut_image(image, threshold = 210, char_distence = 20, padding = 30):
...
def write_images(page, rects, images, info_list, biases = 30):
    ...
    for i in range(len(rects)):
        ...
        for item in info_list:
            ...
            #if item[0] == page+1 and abs(item[1] - x - w//2) < biases and abs(item[2] - y - h//2) < biases:
            if item[0] == page+1 and item[1] > x - biases and item[1] < x + w +biases and item[2] > y - biases and item[2] < y + h + biases:
                filename = item[3] + '.jpg'
...
def phase_pdf_raw(pdf_file):
    ...
    if os.path.exists(filename):
        print(filename, ' exists, delete it. ')
        shutil.rmtree(filename)
    else:
        print(filename, ' is not exist, create it. ')
    ...
    for page in pdf_images:
        ...
        for image in res_images:
            cv.imwrite(output_file%(page_index, 
                                    sec_index, 
                                    res_rects[sec_index-1][0] + res_rects[sec_index-1][2]//2, 
                                    res_rects[sec_index-1][1] + res_rects[sec_index-1][3]//2), 
                       image)
...
def phase_pdf(pdf_file):
    ...
    if os.path.exists(filename):
        print('Directory \"', filename, '\" exists, delete it. ')
        shutil.rmtree(filename)
...
def main():
    ...
    parser.add_argument('-r', '--raw', default = False, action = "store_true",
                        help = '[Optional] phase the pdf and output alll raw block. ')    

    FLAGS = parser.parse_args()

    if FLAGS.raw:
        real_action = phase_pdf_raw
    else:
        real_action = phase_pdf
